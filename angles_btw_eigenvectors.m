...
for nvector=1:450
    ...
    cos_theta = dot(u,conj(v))/( sqrt ( dot(u,conj(u)) )*sqrt( dot(v,conj(v)) ) );
    theta_btw_perturbed_nonperturbed(nvector) = acosd(cos_theta);
end
...
for nvector=1:449
    ...
    %cos_theta = dot(u,v)/(norm(u)*norm(v));
    cos_theta = dot(u,conj(v))/( sqrt ( dot(u,conj(u)) )*sqrt( dot(v,conj(v)) ) );
    theta_btw_perturbed_neighbour(nvector) = acosd(cos_theta);
    
    u(:) = H_nonperturbed_sorted(1,1,:,nvector);
    v(:) = H_nonperturbed_sorted(1,1,:,nvector+1);
    %cos_theta = dot(u,v)/(norm(u)*norm(v));
    cos_theta = dot(u,conj(v))/( sqrt ( dot(u,conj(u)) )*sqrt( dot(v,conj(v)) ) );
    theta_btw_nonperturbed_neighbour(nvector) = acosd(cos_theta);
end
