function [eps_total, mu_total] =...
    ...
    N_intervals_x,N_intervals_y,La,epsilon, int_eps_zz,...
...
eps_inv_zz = zeros(N_total_3,N_total_3);
...
mu_inv_zz = zeros(N_total_3,N_total_3);
...
for j2=1:N_intervals_y
    ...
    eps_xx(row,col)=epsilon(j1,j2)*int_eps_xx_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
    eps_yx(row,col)=epsilon(j1,j2)*int_eps_yx_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
    mu_xx(row,col) =mmu(j1,j2)*int_eps_xx_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
    mu_yx(row,col) =mmu(j1,j2)*int_eps_yx_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
            end
            end
        end
        for k1=1:N_intervals_x
            ...
            col = i2 + (i1-1)*N_total_y3;
    eps_xy(row,col)=epsilon(j1,j2)*int_eps_xy_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
    eps_yy(row,col)=epsilon(j1,j2)*int_eps_yy_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2)); 
    mu_xy(row,col) =mmu(j1,j2)*int_eps_xy_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
    mu_yy(row,col) =mmu(j1,j2)*int_eps_yy_full(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
            end
            end
        end
    end
...
for j2=1:N_intervals_y
    ...
    eps_inv_zz(row,col) = epsilon_inv(j1,j2)*int_eps_zz(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
    mu_inv_zz(row,col) = mmu_inv(j1,j2)*int_eps_zz(m1,m2,i1,i2)/(hx(m1,m1)*hy(m2,m2));
            end
            end
    end
    end
